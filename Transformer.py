...
"""
    ...
    The Bellow part of the code is just to check weather the code is working properly or not 
...
if __name__ == "__main__":
    ...
    # Now creating the english and the russian vocablury
    english_to_idx = {v: k for k, v in enumerate(english_vocabulary)}
    idx_to_english = {k: v for k, v in enumerate(english_vocabulary)}
    hindi_to_idx = {v: k for k, v in enumerate(hindi_vocabulary)}
    idx_to_hindi = {k: v for k, v in enumerate(hindi_vocabulary)}
    ...
    def create_masks(eng_batch, hindi_batch):
        ...
        encoder_padding_mask = torch.full(
            [num_sentences, max_seq_len, max_seq_len], False
        )
        ...
        for idx in range(num_sentences):
            eng_sentence_length, hindi_sentence_length = len(eng_batch[idx]), len(
                hindi_batch[idx]
            )
            eng_chars_to_padding_mask = np.arange(eng_sentence_length + 1, max_seq_len)
            hindi_chars_to_padding_mask = np.arange(
                hindi_sentence_length + 1, max_seq_len
            )
            encoder_padding_mask[idx, :, eng_chars_to_padding_mask] = True
            encoder_padding_mask[idx, eng_chars_to_padding_mask, :] = True
            decoder_padding_mask_self_attention[
                idx, :, hindi_chars_to_padding_mask
            ] = True
            decoder_padding_mask_self_attention[
                idx, hindi_chars_to_padding_mask, :
            ] = True
            decoder_padding_mask_cross_attention[
                idx, :, eng_chars_to_padding_mask
            ] = True
            decoder_padding_mask_cross_attention[
                idx, hindi_chars_to_padding_mask, :
            ] = True
    ...
    x, y = ("Wow!", "Help!", "Jump.", "Jump."), ("वाह!", "बचाओ!", "उछलो.", "कूदो.")
